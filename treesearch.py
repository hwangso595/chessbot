...
class MonteCarloTreeSearch:
    ...
    def reach_leaf_node(self, env):
        ...
        while node.children:
            q = -1 * np.array(list(map(lambda next_node: next_node.stats['q'], node.children)), dtype=np.float32)
    ...
    def reach_leaf_node_parallel(self, env, lock):
        ...
        while node.children:
            ...
            puct = q + u
            if not puct.any():
                action = random.randrange(len(node.children))
            else:
    ...
    def evaluate_node(self, node, model):
        values, policy = model(np.expand_dims(node.state, axis=0).astype(dtype=np.float32))
    ...
    def evaluate_leaves(self, nodes, states, model):
        values, policy = model(states.astype(dtype=np.float32))
